...
/* Component */
const GateItemsRow = ({ formik, index, items, helpers }) => {
  const [products, setProducts] = useState([]);
  const [updateCost, setUpdateCost] = useState(false);
  const { setFieldValue } = useFormikContext();

  const item = useMemo(() => {
    return getIn(formik.values, `gates[${index}].item`);
  }, [index, formik.values]);
  ...
  const updatePrices = useCallback(
    (productName, qty) => {
      ...
      setFieldValue(`gates[${index}].cost`, cost);
      setFieldValue(`gates[${index}].total`, total);
    },
  ...
  const handleChangeItem = (e) => {
    ...
    formik.setFieldValue(`gates[${index}].product`, '');
  ...
  const handleChangeQty = (e) => {
    formik.handleChange(e);
    const productName = getIn(formik.values, `gates[${index}].product`);
        ...
        <TableRow key={index} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
          ...
          <TableCell scope="row" className="fence-product">
            {products.length ? (
              ...
              <FormikTextField id={`gates[${index}].product`} className="fence-product" formik={formik} disabled />
            )}
          </TableCell>
          ...
          <TableCell align="center">
            <FormikTextField id={`gates[${index}].cost`} className="priceFormat" formik={formik} disabled />
          </TableCell>
          <TableCell align="center">
